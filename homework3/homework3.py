...
Original file is located at
    https://colab.research.google.com/drive/13sdfEn0TTPpTT1Co6TQf2uWKPz0khB4a
...
def note_unigram_probability(midi_files):
    ...
    # 
    totalNotes = 0
    for x in note_counts:
        totalNotes += note_counts[x]
    for x in note_counts:
        unigramProbabilities[x] = note_counts[x] / totalNotes
...
def note_bigram_probability(midi_files):
    ...
    for midi_file in midi_files:
        ...
        for i in range(len(notes) - 1):
            prev = notes[i]
            curr = notes[i + 1]
            bigramTransitions[prev].append(curr)

    for prev, next_notes in bigramTransitions.items():
        total = len(next_notes)
        probabilities = [next_notes.count(note) / total for note in next_notes]
        # bigramTransitions[prev] = list(counts.keys()) 
        bigramTransitionProbabilities[prev] = probabilities
...
def sample_next_note(note):
    ...
    # Otherwise, sample the next note based on the probabilities
